...
void SpGEMM_BIN<IndexType, ValueType>::set_rows_offset(IndexType nrows)
{
    ...
    // rows_offset is already allocated in constructor (matching reference implementation)
    ...
    // Set rows_offset for each thread (matching reference implementation)
    rows_offset[0] = 0;
    #pragma omp parallel
    {
        int tid = Le_get_thread_id();

        long long int end_itr = (std::lower_bound(ps_row_nz, ps_row_nz + nrows + 1, average_work * (tid + 1))) - ps_row_nz;
        rows_offset[tid + 1] = end_itr;
        // if (tid == thread_num - 1) rows_offset[tid + 1] = nrows;
    }
