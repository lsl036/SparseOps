    ...
    IndexType csz)
{
    ...
    /* AVX-512: 一致填充到 8 — 用 mask 处理 csz 1..8，取整列 A 与广播的 bv 做向量化 */
    if (csz >= 1 && csz <= 8) {
        ...
        __m512d res = _mm512_fmadd_pd(a_v, bv_v, d_v);
    ...
    if (csz == 8) {
        ...
        __m256d a_lo = _mm256_loadu_pd(aval_slot);
        __m256d d_lo = _mm256_load_pd(dense_slot);
        __m256d a_hi = _mm256_loadu_pd(aval_slot + 4);
    ...
    if (csz == 4) {
        __m256d bv_v = _mm256_set1_pd(bv);
        __m256d a_v = _mm256_loadu_pd(aval_slot);
    ...
    if (csz == 2) {
        ...
        __m128d a_v = _mm_loadu_pd(aval_slot);
    ...
    /* AVX2 下 csz 3,5,6,7: 向下取整到 2,4 或标量展开 */
#pragma GCC unroll 8
    ...
    IndexType csz)
{
    ...
    if (csz == 8) {
        ...
        __m256 a_v = _mm256_loadu_ps(aval_slot);
    ...
    if (csz == 4) {
        ...
        __m128 a_v = _mm_loadu_ps(aval_slot);
...
static inline void accumulate_v8(
    ...
    IndexType csz)
{
    accumulate_v8_double<IndexType>(dense_slot, aval_slot, bv, csz);
}
    ...
    #pragma omp parallel num_threads(bin->allocated_thread_num)
    {
        ...
        for (IndexType cluster_id = start_cluster; cluster_id < end_cluster; ++cluster_id) {
            ...
            if (acc_flag[cluster_id] == 1) {
                ...
                /* 从 A 的一列开始向量化：取一个 B 值 bv 广播，与整列 A 做向量运算，整列结果写回 buf */
                for (IndexType j = col_start; j < col_end; ++j) {
                    t_acol = acol[j];
