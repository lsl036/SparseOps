...
Usage:
    python3 compare_results_vlength.py [--kernel=1|2|hashvlengthcluster_hc_lsh|arrayvlengthcluster_hc_lsh] [--rel-tol=1e-10] [--abs-tol=1e-6]
...
def parse_kernel_arg(kernel_arg):
    ...
    Returns: suffix string (e.g., "hashvlengthcluster_hc_lsh" or "arrayvlengthcluster_hc_lsh")
    """
    if kernel_arg is None:
        return "hashvlengthcluster_hc_lsh"  # Default to kernel 1
...
def get_kernel_display_name(suffix):
    ...
    if suffix == "hashvlengthcluster_hc_lsh":
...
Examples:
  ...
  python3 compare_results_vlength.py --kernel=hashvlengthcluster_hc_lsh
  python3 compare_results_vlength.py --kernel=2
  python3 compare_results_vlength.py --kernel=arrayvlengthcluster_hc_lsh
  python3 compare_results_vlength.py --kernel=1 --rel-tol=1e-12 --abs-tol=1e-8
    ...
    parser.add_argument(
        ...
        type=str,
        default="1",
        help="Kernel to compare: 1 or hashvlengthcluster_hc_lsh (Hash-based variable-length cluster, default), "
             "2 or arrayvlengthcluster_hc_lsh (Array-based variable-length cluster)"
    )
    parser.add_argument(
        "--rel-tol",
        type=float,
