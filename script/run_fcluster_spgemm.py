#!/usr/bin/env python3
"""
Fixed-length Cluster SpGEMM test script.
Runs test_spgemm_flength on multiple datasets with specified kernel.

Usage:
    python3 run_fcluster_spgemm.py [--kernel=1|2] [--test_type=correctness|performance] 
                                    [--iterations=N] [--cluster_sz=N] [--sort=0|1]
    
    --kernel: Select kernel to test (default: 1)
              1: Hash-based fixed-length cluster SpGEMM
              2: Array-based fixed-length cluster SpGEMM (reserved for future)
    ...
    --cluster_sz: Cluster size for fixed-length clustering (default: 8)
    
    --sort: Sort output columns (default: 0, no sort)
"""
...
KERNEL_NAMES = {
    1: "Hash-based fixed-length cluster",
    2: "Array-based fixed-length cluster (reserved)"
}
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Run fixed-length cluster SpGEMM tests on multiple datasets",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python3 run_fcluster_spgemm.py --kernel=1
  python3 run_fcluster_spgemm.py --kernel=1 --test_type=performance
  python3 run_fcluster_spgemm.py --kernel=1 --cluster_sz=16
  python3 run_fcluster_spgemm.py --kernel=1 --test_type=performance --iterations=20
  python3 run_fcluster_spgemm.py --kernel=2 --test_type=correctness
        """
    )
    ...
    parser.add_argument(
        ...
        choices=[1, 2],
        default=1,
        help="Kernel to test: 1 (Hash-based), 2 (Array-based, reserved for future, default: 1)"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help="Cluster size for fixed-length clustering (default: 8)"
    )
    ...
    parser.add_argument(
        ...
        choices=[0, 1],
        default=0,
        help="Sort output columns: 0 (no sort, default) or 1 (sort)"
    )
...
def run_test(dataset, kernel, test_type, iterations, cluster_sz, sort_output):
    """Run test_spgemm_flength for a single dataset."""
    bin_path = os.path.join(SPARSEOPS_ROOT, "build", "test_spgemm_flength")
    ...
    print(f"Cluster Size: {cluster_sz}")
...
def main():
    ...
    # Print configuration
    print("=" * 60)
    print("Fixed-length Cluster SpGEMM Test Script")
    print("=" * 60)
    print(f"Kernel: {args.kernel} ({KERNEL_NAMES[args.kernel]})")
    print(f"Test Type: {args.test_type}")
    print(f"Cluster Size: {args.cluster_sz}")
    print(f"Sort Output: {args.sort}")
