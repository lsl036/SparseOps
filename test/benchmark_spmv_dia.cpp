...
void run_dia_kernels(int argc, char **argv)
{
    ...
    for(int methods = 1; methods <= 1; ++methods){
        msec_per_iteration = test_dia_matrix_kernels(csr, methods, sche_mode, format_convert);
        fflush(stdout);
...
int main(int argc, char** argv)
{
    ...
    if (Index == 0 && precision ==  32){
        ...
        run_dia_kernels<long long, float>(argc,argv);
    }
    else if (Index == 1 && precision == 64){
        run_dia_kernels<long long, double>(argc,argv);
    }
    else{
        usage(argc, argv);
        return EXIT_FAILURE;
