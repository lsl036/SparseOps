...
void run_bsr_kernels(int argc, char **argv)
{
    ...
    for(int methods = 2; methods < 3; ++methods){
        msec_per_iteration = test_bsr_matrix_kernels(csr, methods, sche_mode, format_convert);
        fflush(stdout);
...
int main(int argc, char** argv)
{
    ...
    if (Index == 0 && precision ==  32){
        run_bsr_kernels<int, float>(argc,argv);
    }
    else if (Index == 0 && precision == 64){
        run_bsr_kernels<int, double>(argc,argv);
    }
    else if (Index == 1 && precision ==  32){
        run_bsr_kernels<long long, float>(argc,argv);
    }
    else if (Index == 1 && precision == 64){
        run_bsr_kernels<long long, double>(argc,argv);
    }
    else{
        usage(argc, argv);
        return EXIT_FAILURE;
